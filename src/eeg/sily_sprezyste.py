...
def plt_spring_A(data):
    ...
    if abs(L_0 - b) <= 3 * std_dev:
        print(
            f"L_0 = {L_0:.4f} m odpowiada przecięciu y b = {b:.4f} ± {3 * std_dev:.4f} m"
        )
    else:
...
def plt_spring_B(data):
    ...
    T = data[
        "T"
    ]  # okres [s]    # Metoda B: Badanie zaleznosci okresu drgań od masy zawieszonej na sprężynie
    ...
    if abs(T_mean - b) <= 3 * std_dev:
        print(
            f"Średni okres T = {T_mean:.4f} s odpowiada przecięciu y b = {b:.4f} ± {3 * std_dev:.4f} s"
        )
    else:
        print(
            f"Średni okres T = {T_mean:.4f} s nie odpowiada przecięciu y b = {b:.4f} s"
        )
